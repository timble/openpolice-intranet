...
<script>
    window.addEvent('load', function() {
        Editors.get('commentnew').addEvent('onIsDirty', function() {
            document.getElement('#fora-comment-form input[type=submit]').set('disabled', '');
        });
        Editors.get('commentnew').addEvent('onIsNotDirty', function() {
        	document.getElement('#fora-comment-form input[type=submit]').set('disabled', 'disabled');
        });
...
<div id="fora-comment-form">
	<form action="<?= @route('view=comment') ?>" method="post" class="-koowa-form form-stacked" enctype="multipart/form-data">
	    ...
	    <div class="form-actions">
	    	<input type="submit" class="btn btn-primary" value="<?= @text('Post your comment') ?>" disabled="disabled" />
	    </div>
	</form>
</div>