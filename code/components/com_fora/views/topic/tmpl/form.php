...
<script>
    window.addEvent('domready', (function(){
    	<? if (!$topic->id) : ?>
            new Attachments.Upload({holder: 'fora-topic-form'});
        <? endif ?>

        var holder = document.id('fora-topic-form');
...
<div id="fora-topic-form">
	<div class="frame">
		<div class="content">
			<form action="" method="post" class="-koowa-form form-stacked" enctype="multipart/form-data">
                <input type="hidden" name="action" value="save" />
			    <input type="hidden" name="fora_forum_id" value="<?= $forum->id ?>" />
                
                <div class="spacing">
				    <h2><?= @text(($topic->id ? 'Edit' : 'New').' '.$forum->type) ?></h2>
				    <div class="control-group">
				    	<label class="control-label" for="title"><?= @text('Subject') ?></label>
				    	<div class="controls">
				    	    <input type="text" name="title" class="required" value="<?= @escape($topic->title) ?>" />
				    	</div>
				    </div>
				    <div class="control-group">
				        <? if ($agent) : ?>
				        ...
				        <div class="controls">
				            <input type="text" name="slug" value="<?= $topic->slug ?>" placeholder="<?= @text('Slug') ?>" />
				        </div>
				        <? endif ?>
				    </div>
				    <div class="control-group">
				    	...
				    	<div class="controls">
	                        <?
	                            ...
	                            echo $controller->name('text')->data($topic->text)->toggle(false)->codemirror($agent)->display();
	                        ?>
	                    </div>
				    </div>
